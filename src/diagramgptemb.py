...
try:
    ...
    st.error("API key for DashScope is missing. Please add the DASHSCOPE_API_KEY to Streamlit secrets.")
    st.stop()
except Exception as e:
    st.error(f"Failed to initialize OpenAI client: {str(e)}")
    st.stop()

def generate_diagram_prompt(user_input, selected_providers, color_scheme, layout_direction):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        try:
            ...
            if match:
                ...
                # Return fallback image instead of showing error
            ...
            try:
                ...
                # Return fallback image without showing error messages
                return get_fallback_image()
            finally:
...
def get_fallback_image():
    """Helper function to load and return a fallback image"""
    fallback_path = "temp/basic.jpg"
    if not os.path.exists(fallback_path):
        fallback_path = "temp/general.png"
